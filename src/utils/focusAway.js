...
//
// ★5초인 이유(사용자 결정 2026-07-30)★ — 판정 기준(10초)까지 기다리면 오알림이 구조적으로
// 사라지지만 "나갔는데 한참 조용한" 느낌이 커서, **알림창·앱 전환 같은 순간 전환(1~2초)만
// 걸러내는 최소값**으로 잡았다. 대신 5~10초 사이에 돌아오면 '알림은 왔는데 이탈은 안 세는'
// 경우가 드물게 남는다 — 그 알림은 복귀 시 트레이에서 자동으로 내려간다(cancelAwayNudges).
// 더 줄이면(0~2초) 원래 증상이 그대로 돌아오므로 내리지 말 것.
