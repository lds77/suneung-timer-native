...
export default function SettingsScreen() {
  ...
  // Modal 열릴 때 Row touch-up이 backdrop으로 새어들어가는 걸 방지
  const backdropLocked = useRef(false);
  const openPicker = (setter) => { backdropLocked.current = true; setter(true); };
  const onPickerShow = () => { backdropLocked.current = false; };
  useEffect(() => {
      ...
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, isTablet && { maxWidth: tabletMaxW, alignSelf: 'center', width: '100%' }]}
          ...
          {(() => {
            ...
            return (
              <Row T={T} label="집중탭 헤더 배경"
                ...
                onPress={() => openPicker(setShowHeaderBgPicker)}
              />
            );
          })()}
        </Section>
        ...
        <Section T={T} title="목표">
          <Row T={T} label="일일 목표 시간"
            ...
            onPress={() => openPicker(setShowGoalPicker)}
          />
          <Row T={T} label="학교급"
            right={<View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
            ...
            onPress={() => openPicker(setShowSchoolPicker)}
          />
        </Section>
            ...
            <>
              ...
              {app.settings.dailyReminderEnabled && (
                <Row T={T} label="알림 시각"
                  ...
                  onPress={() => openPicker(setShowReminderPicker)}
                />
              )}
              <Row
                T={T}
          ...
          {(() => {
            ...
            return (
              <Row T={T} label="테마 컬러"
                ...
                onPress={() => openPicker(setShowColorPicker)}
              />
            );
          })()}
          {(() => {
            ...
            return (
              <Row T={T} label="타이머 스타일"
                ...
                onPress={() => openPicker(setShowStylePicker)}
              />
            );
          })()}
          ...
          {(() => {
            ...
            return (
              <Row T={T} label="글꼴"
                ...
                onPress={() => openPicker(setShowFontPicker)}
              />
            );
          })()}
        </Section>
...
{/* 사용 가이드 */}
      ...
      {/* 테마 컬러 피커 */}
      <Modal visible={showColorPicker} transparent animationType="slide" onShow={onPickerShow}>
        <TouchableOpacity style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.45)' }]} activeOpacity={1} onPress={() => { if (!backdropLocked.current) setShowColorPicker(false); }} />
        <View style={{ position: 'absolute', bottom: 0, left: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, right: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, maxHeight: isLandscape ? '95%' : '92%', backgroundColor: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 36 }}>
      ...
      {/* 헤더 배경 피커 */}
      <Modal visible={showHeaderBgPicker} transparent animationType="slide" onShow={onPickerShow}>
        <TouchableOpacity style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.45)' }]} activeOpacity={1} onPress={() => { if (!backdropLocked.current) setShowHeaderBgPicker(false); }} />
        <View style={{ position: 'absolute', bottom: 0, left: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, right: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, maxHeight: isLandscape ? '95%' : '92%', backgroundColor: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 36 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20, paddingBottom: 16 }}>
      ...
      {/* 스타일 프리셋 피커 */}
      <Modal visible={showStylePicker} transparent animationType="slide" onShow={onPickerShow}>
        <TouchableOpacity style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.45)' }]} activeOpacity={1} onPress={() => { if (!backdropLocked.current) setShowStylePicker(false); }} />
        <View style={{ position: 'absolute', bottom: 0, left: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, right: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, maxHeight: isLandscape ? '95%' : '92%', backgroundColor: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 36 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20, paddingBottom: 16 }}>
      ...
      {/* 글꼴 피커 */}
      <Modal visible={showFontPicker} transparent animationType="slide" onShow={onPickerShow}>
        <TouchableOpacity style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.45)' }]} activeOpacity={1} onPress={() => { if (!backdropLocked.current) setShowFontPicker(false); }} />
        <View style={{ position: 'absolute', bottom: 0, left: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, right: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, maxHeight: isLandscape ? '95%' : '92%', backgroundColor: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 36 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20, paddingBottom: 16 }}>
      ...
      {/* 일일 목표 시간 피커 */}
      <Modal visible={showGoalPicker} transparent animationType="slide" onShow={onPickerShow}>
        <TouchableOpacity style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.45)' }]} activeOpacity={1} onPress={() => { if (!backdropLocked.current) setShowGoalPicker(false); }} />
        <View style={{ position: 'absolute', bottom: 0, left: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, right: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, maxHeight: isLandscape ? '95%' : '92%', backgroundColor: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 36 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20, paddingBottom: 12 }}>
      ...
      {/* 학교급 피커 */}
      <Modal visible={showSchoolPicker} transparent animationType="slide" onShow={onPickerShow}>
        <TouchableOpacity style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.45)' }]} activeOpacity={1} onPress={() => { if (!backdropLocked.current) setShowSchoolPicker(false); }} />
        <View style={{ position: 'absolute', bottom: 0, left: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, right: isTablet ? Math.max(0, (winW - tabletMaxW) / 2) : 0, maxHeight: isLandscape ? '95%' : '92%', backgroundColor: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 36 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20, paddingBottom: 12 }}>
