...
//   tint, textColor, subColor: string — 테마 색상(hex)
  ...
  return {
    ...
    compactTrailing:
      props.mode === 'none' ? (
        ...
        <Text
          timerInterval={{ lower: new Date(props.startMs), upper: new Date(upperMs) }}
          countsDown={props.mode === 'down'}
          modifiers={[font({ size: 14, weight: 'semibold' }), monospacedDigit(), foregroundStyle(props.tint), frame({ width: 56 })]}
        />
      ),
    ...
    expandedCenter: (
      <VStack alignment="center" spacing={2}>
        ...
        <Text modifiers={[font({ size: 12 }), foregroundStyle(props.subColor)]}>
          {props.subtitle}
        </Text>
      </VStack>
    ),
