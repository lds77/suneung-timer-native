...
1. ~~iOS SecureStore 셔임 + 토큰 마이그레이션~~ **구현됨** (2026-07-19,
   `src/utils/durableAuthStorage.js` + studyRoom.js 주입, 테스트 5케이스 — 재설치
   복구/미러 마이그레이션/실패 내성/구빌드 폴백. expo-secure-store ~56.0.4 설치됨)
2. ~~Android allowBackup 전환~~ **구현됨** (2026-07-19 사용자 승인, app.config.js)
3. EAS 빌드(51+/vc60+) → 실기기 검증 — **미완, 다음 빌드 때**:
   - "설치→방 입장→삭제→재설치→같은 uid 확인" (iOS/안드 각각)
   - iOS: SecureStore 값 2048바이트 초과 경고 나오는지 + 실제 저장되는지
   - 안드: 자동백업은 24h 주기라 `adb shell bmgr backupnow com.yeolgong.timer`로 강제 후 테스트
4. B단계(소셜 링크)는 1~2 배포 후 사용자 피드백 보고 결정
