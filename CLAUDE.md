...
- **'화면 끄기/잠금'은 이탈이 아니다** (양 플랫폼, 다음 네이티브 빌드~): 화면을 끄면 앱이
  ...
  - **안드**: `modules/screen-pin`의 `screenState()`(PowerManager.isInteractive + SCREEN_ON/OFF
    브로드캐스트 시각) → 즉시 판정. 우회 방지: **잠금을 푼 뒤** 10초 넘게 앱으로 안 돌아오면
    그 구간만 이탈로 계산
    - ★기준점은 화면 켬이 아니라 **잠금해제**(`ACTION_USER_PRESENT` → `lastUnlockAt`)★ (2026-07-30).
      다른 앱은 잠금을 풀어야 열 수 있으므로 잠금화면 체류·패턴 입력은 이탈이 아니다. 화면 켬
      기준이던 시절엔 **잠금화면을 12초만 들여다봐도 이탈 1회가 조용히 찍혔다**(화면 꺼짐 감지는
      정상이라 markAway를 안 거치고 복귀 시 재계산에서만 확정 → **알림도 챌린지도 없이 카운트만** 증가).
      기기의 '화면이 꺼진 후 잠금 시간' 설정 탓에 짧게 끄면 잠금이 안 걸려 재현이 안 되고 오래 끄면
      재현돼, **'오래 꺼서 생긴 문제'로 오인하기 쉽다** — 재현 조건은 끔 시간이 아니라 잠금화면 체류 시간
    - 잠금 미설정 기기는 `USER_PRESENT`가 안 오므로 화면 켬 시각으로 폴백(구빌드도 동일)
