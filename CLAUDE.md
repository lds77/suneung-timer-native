...
- **현재 버전**: 1.0.40 (iOS buildNumber 57 / Android versionCode 72).
  라이브: **iOS 1.0.40 (2026-08-04 승인)** / **Android 1.0.39(vc70, 2026-08-05 승인)**.
  iOS는 1.0.37~1.0.39를 출시하지 않았다 — 1.0.37·1.0.38은 미빌드(무료 할당량 소진),
  1.0.39(build56)는 **CallKit/중국 리젝**으로 폐기하고 build57(1.0.40)로 직행했다.
  Android는 **vc72(1.0.40)가 미빌드** — 음성 메모·백업 zip 등 네이티브 4건이 안드에 없다.
  ★**vc70 = 2026-08-01 10:01 HEAD 기준**★ — 그 뒤 커밋(시간 입력 개편·zip 백업·음성 메모·
  계획 5분 규칙 통일·연속모드 결과 모달)은 **안드 출시본에 없다**. 문구·웹사이트를 고칠 때
  이 경계가 판단 기준이다.
...
| Android | Google Play 라이브 **1.0.39(vc70)** — **2026-08-05 승인**(모드 이름 통일·3지선다·화면 끄기/전화 이탈 면제·AwayWatch가 이때 나갔다). 다음이 **vc72(1.0.40) — 미빌드**. ★안드로이드만 네이티브 3건(RECORD_AUDIO·zip 백업·첨부 백업 제외)이 미출시라 OTA로 못 메운다 — 4.10절대로 **1.0.39 런타임 OTA 금지**★ |
...
| 브랜치 | 작업은 `sdk56`, 배포 승인 후 `main` 머지 — **1.0.35 이후 머지 보류 중**. iOS 1.0.40·안드 1.0.39가 모두 승인돼(08-05) **조건은 완전히 풀렸고 머지만 남음**(사용자 승인 필요) |
