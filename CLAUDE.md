...
- **현재 버전 = 라이브**: **iOS 1.0.41 (build58, 2026-08-26 승인)** / **Android 1.0.40 (vc72, 2026-08-09 승인)**.
  ★**1.0.41은 코드 변경이 0인 메타데이터 릴리스다**★ — App Store 부제·키워드가 **새 버전
  제출과 함께만** 바뀌기 때문에 빌드했을 뿐, 새로 나가는 기능은 없다(build57 이후 앱 소스는
  전부 OTA 3회로 이미 라이브). ★★그래서 **런타임이 갈라졌다** — iOS 1.0.41 / 안드 1.0.40.
  다음 OTA는 **런타임별로 두 번 쏴야 안드까지 닿는다**★★ (08-09~08-26 동안만 하나로 닿았다).
  안드 `versionCode: 74`는 예약 번호일 뿐 **아직 빌드하지 않았다**.
  iOS는 1.0.37~1.0.39를 출시하지 않았다 — 1.0.37·1.0.38은 미빌드(무료 할당량 소진),
  1.0.39(build56)는 **CallKit/중국 리젝**으로 폐기하고 build57(1.0.40)로 직행했다.
  ※플랫폼이 갈렸던 구간의 경계는 **vc70 = 2026-08-01 10:01 HEAD**였다 — 지금은 vc72가
  그 뒤 커밋을 전부 실어 나갔으므로 **더 이상 판단 기준이 아니다**(옛 문서를 읽을 때만 참고).
...
7. **5분(300초) 미만 세션 미기록** (계획·할일 연결 시 30초), 30초 미만은 밀도 100점 고정.
   ...
   ★**모달이 안 뜨는 종료는 토스트가 유일한 피드백이다**(`stopFeedbackToast`, 2026-08-04)★ —
   예전엔 완료 카드가 밀도·티어를 인라인으로 보여준다고 적어뒀지만, 그 카드를 그리던
   `FocusScreen.renderTimer`는 **타이머 뷰 개편(미니/기본/전체) 때 호출부를 잃어 현재 미사용**이다.
   그래서 5분 미만·휴식 중 종료는 화면에서 타이머만 조용히 사라졌다(실기기 제보로 발견).
   ※`renderTimer`와 그에 딸린 한줄 메모 모달은 **도달 불가 코드** — 살릴지 지울지는 미결
   - ★**계획 타이머도 같은 5분 규칙을 따른다**(2026-08-01)★ — 예전엔 **계획 목표의 80%에
     도달해야만** 모달이 떴고 그 계획의 오늘 세션을 묶어(`planSessionIds`) 자기평가를 일괄
     적용했다. 그래서 목표 60분에 40분(66%)을 공부하면 **자기평가 기회 자체가 없어**
     밀도 보너스(최대 +3)를 못 받고 점수가 낮게 굳었다(같은 40분을 계획 없이 돌리면 떴다).
     이제 세션 단위(`sessionId`)로 넘겨 시간 수정·개별 삭제도 된다.
     **80%는 '계획 달성!' 축하 문구에만 쓴다**(`planAchieved` — 집중탭 계획 카드·위젯의
     완료 기준과 같은 값). 판정은 `useAppState.planResultExtras`
...
- 🔥모드 이탈 시: 이탈 알림 + 30초/1분/3분/5분 에스컬레이팅 넛지(복귀 시 취소, countdown 잔여시간 초과분 미예약),
  ...
  - **이탈로 인정하는 최소 시간은 `focusAway.awayMinMs(Platform.OS)` 하나뿐이다** — 앱 전환 경로,
    화면 끄기 후 잠금해제 경로, iOS 지연 판정이 모두 이 값을 쓴다. 예전엔 useAppState에
    같은 값이 **따로 하드코딩**돼 있어 한쪽만 고치면 경로마다 기준이 갈렸다(2026-07-30 일소).
    **판정 시간을 숫자로 다시 박지 말 것**
  - ★**판정 = 첫 이탈 알림 + `AWAY_NOTIF_GRACE_SEC`(10초)**★ (2026-08-01) — 판정을 숫자로 두지 않고
    **첫 알림에서 파생**시킨다. 안드 5+10=**15초**, iOS 20+10=**30초**.
    - 이렇게 바꾼 이유: **iOS는 이 관계가 뒤집혀 있었다**(알림 20초 > 판정 15초) →
      **알림을 본 순간 이미 이탈이 확정**이라 "돌아올 기회"가 아예 없었다
      (실기기 제보 2026-08-01 "20초 후 바로 복귀해도 이탈 1회"). 안드는 맞아 있어서
      같은 코드인데 플랫폼마다 체감이 정반대였다
    - **iOS 첫 알림을 안드처럼 5초로 앞당길 수는 없다** — 잠금 감지에 약 10초가 걸려,
      앞당기면 화면을 끌 때마다 잠금화면에 헛알림이 떴다 사라진다(10분 잠금 무알림을
      실기기로 확인한 그 동작이 깨진다). 그래서 판정을 뒤로 미루는 쪽을 택했다
    - 대가: **iOS는 30초 미만 앱 전환을 이탈로 세지 않는다**(안드 15초). 울트라모드도
      같은 하한에 걸려 iOS에서 더 느슨하다 — 'iOS 한계(수용)'의 연장
...
- **'화면 끄기/잠금'은 이탈이 아니다** (양 플랫폼, 다음 네이티브 빌드~): 화면을 끄면 앱이
  ...
  - ★**'다른 앱 → 화면 끔' 순서는 이제 잡는다**(2026-08-01)★ — 예전엔 잠금이 감지되면
    그 배경 구간을 **통째로** 면제해서, 다른 앱으로 나갔다가 **이탈 알림이 오기 전에 화면을 끄면
    알림도 안 오고 이탈도 안 잡혔다**(실기기 제보). 배경 진입~잠금 사이는 명백히 다른 앱을 쓴
    시간이므로 그 구간만 잘라 센다(`focusAway.iosAwayBeforeLockMs`, 감지 지연 10초 차감)
    - 알림 쪽은 네이티브가 판단한다 — **첫 알림이 나갈 시각(20초)을 넘겨 감지된 잠금은
      예약을 취소하지 않는다**(`LOCK_CANCEL_WINDOW_SEC`, JS `IOS_AWAY_NOTIF_DELAY_SEC`와 일치
      — 테스트가 Swift를 읽어 대조). 취소는 원래 '앱을 만지고 곧바로 끈' JS 오판을 되돌리려는
      것이고, 그 경우는 간격이 감지 지연분(약 10초)뿐이라 이 창 안에 들어온다
  - ★**'화면 끔 → 잠금 해제 → 다른 앱' 순서는 여전히 못 잡는다**★ — 잠금이 감지된 뒤 앱이
    정지돼 **잠금 해제 시각을 알 방법이 없다**(안드는 `lastOnAt`/`lastUnlockAt`으로 막음).
    백그라운드 예산이 약 30초뿐이라 몇 분 뒤 해제는 관측 불가. **exam 무결성이 iOS에서 더 약함**
    - ※"잠금 후 N분 넘게 안 돌아오면 이탈" 같은 시간 제한을 넣지 말 것 — 화면 끄고 종이책·
      문제집을 보는 것이 이 면제의 **존재 이유**라 그 사용자를 벌주게 된다
...
- **익명 응원은 익명이 설계 의도** — 보낸 사람을 남기면 아는 사이끼리만 주고받게 되어 취지가 죽는다.
  되돌리지 말 것
- 라운지 안전장치: 사용자 숨기기 + 신고 + 닉네임 필터 (**양 플랫폼 출시됨**)
...
## 출시 현황 (2026-08-26 기준 — 라이브 iOS 1.0.41 / Android 1.0.40)

> ※이 표는 쉽게 낡는다. 진행 중인 릴리스의 최신 상태는 항상 `docs/release-next-build-checklist.md`를 볼 것.

| 항목 | 내용 |
|------|------|
| iOS | App Store 라이브 **1.0.41(build58)** — **2026-08-26 승인**(제출 08-25, 하루 만). ★**코드 변경 0의 메타데이터 릴리스**★ — 부제·키워드·전체 설명 3종 교체가 전부이고 기능 델타는 없다(build57 이후 앱 소스는 OTA 3회로 이미 라이브). 여기서부터 **키워드 8주 동결 / 판정 10월 초**. 직전 1.0.40(build57)은 08-04 승인. 1.0.39(build56)는 CallKit/중국 리젝으로 폐기(중국 본토 배포 해제 상태 유지). TestFlight 외부 링크: `https://testflight.apple.com/join/dsNaK9kb` |
...
| 브랜치 | 작업은 `sdk56`, 배포 승인 후 `main` 머지 — 릴리스 머지는 2026-08-05에 했다(`2a0328d`). 그 뒤 **양쪽이 문서에서 갈라져 있다**: `main`에만 웹사이트·블로그 지침서 최신본, `sdk56`에만 체크리스트·CLAUDE.md 최신본 → **양방향 머지 필요**(사용자 승인 필요) |
...
15. **기능이 바뀌거나 스토어 상태가 바뀌면 `docs/blog-guide.md`를 같은 작업에서 갱신하고
    코워크 사본을 통째로 덮어쓴다** — ★사용자가 매번 요청하지 않아도 내가 챙긴다(2026-08-05 위임)★
    - 사본: `C:\Users\가나다라\OneDrive\문서\카카오톡 받은 파일\구글열공\CLAUDE.md`.
      ★**손대기 전에 `diff`부터**★ — 사본에서만 늘어나는 것이 있다(발행 글 대장·스크린샷 목록).
      원본에 병합한 뒤 통째로 덮어쓸 것(부분 수정 금지). 08-04·08-05 연속으로 갈라져 있었다
    - **블로그 세션은 이 대화를 물려받지 않는다 — 파일에 적힌 것만 안다.** 그래서 지침서가
      낡으면 없는 기능을 소개하거나 옛 이름으로 글이 나간다
    - 갱신 기준 두 가지:
      ① **코드에 들어갔지만 미출시** → 0번 상태표에 그렇게 적고 **글에 쓰지 말 것**으로 표시
      ② **스토어 승인** → '출시됨'으로 승격. ★승인 사실은 사용자만 알 수 있으므로
         "승인됐어" 한마디만 주면 나머지는 내가 한다★ (그 판단 근거는 릴리스 체크리스트의
         '어느 커밋까지 그 빌드에 들어갔나' — 날짜나 기억으로 하지 말 것)
