...
- **'화면 끄기/잠금'은 이탈이 아니다** (양 플랫폼, 다음 네이티브 빌드~): 화면을 끄면 앱이
  ...
  - **안드**: `modules/screen-pin`의 `screenState()`(PowerManager.isInteractive + SCREEN_ON/OFF
    ...
    - ★**이탈 시작 시각은 역산하지 말고 배경에서 직접 관측한다**★ (2026-07-30, 2차 제보 대응).
      화면을 끄고 내려간 뒤 **잠금화면에서 곧장 다른 앱으로 가버리면 앱은 `active`를 한 번도 못
      받아** 아래 ①~④ 역산에만 의존하게 되는데, 그 역산이 놓치는 구멍이 있었다 — **화면을 짧게
      꺼서 키가드가 아예 안 걸리면 `ACTION_USER_PRESENT`가 발생하지 않아** `lastUnlockAt`이 영영
      비고, 규칙 ③(방금 푼 것으로 간주)에 걸려 **몇 분짜리 이탈이 통째로 면제**됐다
      (제보: "화면 끄고 조금 있다가 알림창에서 다른 앱으로 가면 첫 번째는 이탈이 안 잡힌다").
      안드는 백그라운드에서도 JS가 도므로(100ms 틱과 같은 조건) `AWAY_WATCH_INTERVAL_MS`(2초)마다
      `screenState()`를 직접 조회해 **'화면이 켜지고 잠금까지 풀린 시각'을 관측**한다
      (`focusAway.js`의 `awayWatchStep`/`awayWatchAwayMs`, 테스트 有). 잠금이 풀린 뒤 10초가
      지나도록 안 돌아오면 그 자리에서 이탈 확정 + 알림/넛지 발송 — 복귀를 기다리지 않으므로
      **이 경로에서도 이탈 알림이 제때 간다**(예전엔 알림도 챌린지도 없이 조용히 넘어갔다).
      관측이 한 번도 못 돌았으면(프로세스 정지·`keyguardLocked`를 못 주는 구빌드) `null` →
      아래 역산으로 폴백한다. ※`readScreenState()`가 **`keyguardLocked`가 없는 빌드에 null을 주는 건
      의도**다 — 잠금 여부를 모르는 채 관측하면 잠금화면 체류를 이탈로 오판한다(51e807f 재발)
